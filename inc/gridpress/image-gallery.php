...
<?php if ($group): ?>
	...
	<div class=" image-gallery <?php echo $class; ?>">
		<?php
			$groups = array();
			$image_by_group = array();
			foreach ($images as $image) {
				$group_name = str_replace(' ', '-', strtolower(rtrim((get_post_meta($image['id'], 'group_name', true)))));
				$image_by_group[$group_name][] = $image;
			...
			foreach($groups as $key) {
				if(array_key_exists($key,$image_by_group)) {
					$ordered[$key] = $image_by_group[$key];
					unset($image_by_group[$key]);
				}
			}
			$images_grouped =  $ordered + $image_by_group;

			foreach($images_grouped as $group => $images):
				foreach ($images as $image) :
				...
				$image_group_thumbnail  = get_post_meta($image['id'], 'group_thumbnail', true);
				if (!$image_url)
				{
		...
		?>
			<a class="image-gallery-anchor-thumb <?php echo $clear; ?> <?php echo $show = $image_group_thumbnail ? '' : ' hidden-thumbnail' ?>" rel="<?php echo $group; ?>" href="<?php echo $image['url']; ?>" title="<?php echo $image['caption']; ?>">
